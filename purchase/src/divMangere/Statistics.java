...
public class Statistics {
	...
	public void getStatistics() throws Exception{
		...
		String position = s[0] ;
		String sql = "SELECT requestcount.Count,requestcount.RealCount,requestcount.RequestAccount,requestcount.RealAccount,`user`.Position"
				+" FROM requestcount,`user`"
				+" WHERE requestcount.UserID=`user`.UserID AND `user`.Position LIKE '"+position+"%'";
		ResultSet rs = db.select(sql) ;
		while(rs.next()){
